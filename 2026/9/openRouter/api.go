...
func fetchCandidates(c *http.Client) ([]candidate, catalogStats, error) {
   ...
   for _, m := range resp.Data.Models {
      ...
      cd := candidate{slug: m.Permaslug, contextLength: m.ContextLength}
...
func filterReport(st catalogStats) string {
   ...
   fmt.Fprintf(&b, "  1. context >= %d tokens   server-side, the `context` query param on models/find\n", minContext)
   ...
   fmt.Fprintf(&b, "\n")
   fmt.Fprintf(&b, "after filter 1 (server):               %6d models\n", st.AfterContext)
   fmt.Fprintf(&b, "  dropped by filter 2 (no weights):    %6d\n", st.DroppedClosed)
   fmt.Fprintf(&b, "  dropped by filter 3 (duplicate slug):%6d\n", st.DroppedDuplicate)
   fmt.Fprintf(&b, "  candidates:                          %6d\n",
...
type catalogModel struct {
   ...
   // query parameter filters on. Display only here.
